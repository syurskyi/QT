...
class LauncherGUI(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Settings and json file read/write stuf
    ...
    def _setup_header(self):
        ...
        self._workspace_cb.view().setDragDropMode(QtWidgets.QAbstractItemView.DragOnly)
    ...
    def _setup_edit_options(self):
        """Create add / delete buttons with menus and overrides"""
    ...
    def _setup_apps(self):
        ...
        self._app_lw.setDragDropMode(QtWidgets.QAbstractItemView.InternalMove)
        return self._app_lw
    ...
    def _setup_connections(self):
        """Create connections between combobox, apps, edit to do stuff"""
        self._workspace_cb.currentIndexChanged.connect(self._workspace_changed)
        self._edit_btn.clicked.connect(self._edit_toggle)
        self._workspace_cb.view().pressed.connect(self._dragging_workspace)
        self._app_lw.itemPressed.connect(self._dragging_app)

    # ================ DISPLAY ========================================================================================

    def _populate_workspaces(self):
        """Clear and Populate the workspace combobox, disconnect and reconnect signal to avoid unintended refresh of populate apps."""
        self._workspace_cb.currentIndexChanged.disconnect(self._workspace_changed)
    ...
    def _populate_apps(self):
        """Clear and Populate the apps list widget, set each application icon and name."""
        self._app_lw.clear()
        workspace = self.get_workspace()
        for app_name in self._launcher.get_app_names(workspace):
            item = QtWidgets.QListWidgetItem(self._app_lw)
            icon = self._launcher.get_app_icon(workspace, app_name)
            if icon:
    ...
    def _edit_toggle(self):
        """Whenever button is clicked, set add/del button vis, set drag drop and connect/disconnect run apps signal"""
    ...
    def _add_workspace(self):
        """Prompt user with line edit input dialog for new name, store if valid, populate, then select it."""
        text = QtWidgets.QInputDialog.getText(self, 'New WORKSPACE', 'What would you like to title the new WORKSPACE?')[0]
        if text:
    ...
    def _add_app(self):
        """Present user with instructions ONCE, then manditory file dialog for app, optional dialog for icon,
    ...
    def _file_dialog(self, msg, path):
        """Default File dialog for Pyside, Pyside2, PyQt5 return file path and filter for the dialog.
        Only need the path"""
        return QtWidgets.QFileDialog.getOpenFileName(self, msg, path)[0]

    def _file_dialog_pyqt5(self, msg, path):
        """File dialog for PyQt4 only returns file path as QString, must be converted to a string"""
    ...
    def _dragging_workspace(self, index):
        """When the user begins dragging a workspace, store the index of the item being dragged and note that it is a workspace."""
        ws = str(self._workspace_cb.itemText(index.row()))
        self._dragging = ('workspace', ws)

    def _dragging_app(self, item):
        """When the user begins dragging a application, store the item text and note it's an app"""
    ...
    def _save_settings(self):
        """Reorder the apps if necessary, then save the user settings to QSettings, and store the tool data to json file."""
        count = self._app_lw.count()
        names = [self._app_lw.item(i).text() for i in range(count)]
        workspace = self.get_workspace()
        self._launcher.reorder_apps(workspace, names)
    ...
    def _load_settings(self):
        """If there is a QSettings file and the json file exists, read in the json file, then make selections based on the qsettings."""
        if 'CurrentWorkspace' in self._settings.allKeys() and \
            ...
            self._launcher.add_workspace('Default_WS')
            self._populate_workspaces()

    def _get_settings(self):
        """Default get settings for x, y position for, tool and current workspace"""
    ...
    def _get_settings_27_pyqt4(self):
        """This specific version of PyQt and python needed to be handled differently."""
...
if __name__ == '__main__':
    ...
    # Create an instance of tool and run i
    ex = LauncherGUI()
    ex.show()
    sys.exit(app.exec_())
